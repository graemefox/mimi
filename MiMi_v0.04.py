...
if __name__ == "__main__":
    ...
    if sys.version_info[0] > 3:
        raise Exception("MiMi must be run using Python 2.x to run correctly.\n")
    try:
        import Bio
    except ImportError:
        print("Please ensure that Biopython has been installed for Python 2.x\n")
    ...
    if len(diff_in_motif_size) > 0:
        ...
        with open(wd + "/MiMi_output/MiMi_output_all_loci.txt", 'w') as final_output:
    ...
    left_side_trim = []
    with open(wd + "/MiMi_output/MiMi_output_all_loci.txt", 'r') as mimi_output:
        next(mimi_output)
        for line in mimi_output:
            record_dict = SeqIO.parse(wd + "/MiMi_output/Alignments/" \
    ...
    with open(wd + "/MiMi_output/MiMi_output_all_loci.txt", 'r') as mimi_output:
        ...
        for line, trim_pos in zip(mimi_output, left_side_trim):
            with open(wd + "/MiMi_output/Alignments/" + line.split("\t")[0] \
    ...
    with open(wd + "/MiMi_output/MiMi_output_all_loci.txt", 'r') as mimi_output:
        ...
        for line in mimi_output:
            ...
            record_dict2 = SeqIO.parse(wd + "/MiMi_output/Alignments/" \
                            + line.split("\t")[0] + ".trimmed", "fasta")
    ...
    with open(wd + "/MiMi_output/MiMi_output_all_loci.txt", 'r') as mimi_output:
        ...
        for line, rev_primer, full_length_cons, partial in zip(mimi_output, conserved_reverse_primer, full_length_perfect, partial_primer_match):
            ...
            record_dict = SeqIO.parse(wd + "/MiMi_output/Alignments/" + line.split("\t")[0] + ".trimmed", "fasta")
            average = []
            for record in record_dict:
                ### measure the overall alignment "quality"
    ...
    with open(wd + "/MiMi_output/MiMi_output_filtered_loci.txt", 'w') as filtered_output:
    ...
    with open(wd + "/MiMi_output/MiMi_loci_filter.log", 'w') as filter_log:
